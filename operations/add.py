...
def Add_Object(
    ...
    if not (opt.dilation_iter_num>0 or opt.use_max_min):
        if preloaded_model:
            # load SAM
    ...
    if '<NULL>' in place:
        ...
        # old mask
        place_box = match_sam_box(place_mask, sam_seg_list, use_max_min=opt.use_max_min, use_dilation=(opt.erosion_iter_num>0), dilation=opt.erosion, dilation_iter=opt.erosion_iter_num) # only mask input -> extract max-min coordinates as bounding box
        if opt.use_ratio:
    ...
    for i in range(num):
        ...
        box_example = match_sam_box(mask_example, sam_seg_list, use_max_min=opt.use_max_min, use_dilation=opt.ersion_iter_num>0, dilation=opt.erosion, dilation_iter=opt.erosion_iter_num)  # only mask input -> extract max-min coordinates as bounding box
